...
clearTokenLink.addEventListener('click', (e) => {
  e.preventDefault();
  if (confirm('Token\'ý silmek istediðinizden emin misiniz?')) {
...
captureBtn.addEventListener('click', async () => {
  try {
    ...
    captureBtn.textContent = '? Yakalýyor...';
    showStatus('Ýlan bilgileri çekiliyor...', 'info');
    ...
    if (!isLinkedIn && !isKariyer && !isIndeed && !isSecretcv) {
      throw new Error('Bu sayfa desteklenmiyor. Lütfen LinkedIn, Kariyer.net, Indeed veya Secretcv iþ ilaný sayfasýnda olduðunuzdan emin olun.');
    }
    const results = await chrome.scripting.executeScript({
      target: { tabId: tab.id },
      func: () => {
    ...
    if (!job || !job.companyName || !job.position) {
      throw new Error('Ýlan bilgileri çekilemedi. Sayfa tam yüklendi mi?');
    }
    capturedJob = {
      ...job,
      jobUrl: tab.url
    ...
    showStatus('? Ýlan baþarýyla yakalandý!', 'success');
  } catch (error) {
    console.error('Capture error:', error);
    showStatus('? ' + error.message, 'error');
  } finally {
    captureBtn.disabled = false;
    captureBtn.textContent = '?? Ýlaný Yakala';
  }
});
saveBtn.addEventListener('click', async () => {
  try {
    ...
    if (!authToken) {
      throw new Error('Token bulunamadý! Lütfen JobTrackr\'da giriþ yapýn.');
    }
    ...
    if (!response.ok) {
      ...
      throw new Error(error.error?.message || 'Kaydetme baþarýsýz');
    }
    showStatus('? JobTrackr\'a baþarýyla kaydedildi!', 'success');
    setTimeout(() => {
      capturedJob = null;
